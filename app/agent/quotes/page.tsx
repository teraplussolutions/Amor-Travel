    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginBottom: 16, alignItems: "center" }}>
        ...
        <button onClick={() => exportPDF({
          titleMk: "Понуди", titleEn: "Quotes",
          columns: [
            { mk: "Код", en: "Code" }, { mk: "Клиент", en: "Client" }, { mk: "Дестинација", en: "Destination" },
            { mk: "Датум", en: "Date" }, { mk: "Вкупно (€)", en: "Total (€)" }, { mk: "Статус", en: "Status" }
          ],
          rows: filtered.map(q => {
            const cl = q.clients as {first_name:string;last_name:string}|undefined;
            return [q.code, cl ? `${cl.first_name} ${cl.last_name}` : "—",
              q.destination ?? "—", q.departure_date ?? "—",
              Number(q.total_eur ?? 0).toFixed(2), q.status];
          }),
          lang: pdfLang,
          subtitle: `${lang === "mk" ? "Вкупно понуди" : "Total quotes"}: ${filtered.length}`,
