...
export default async function TripDetailPage({ params }: TripDetailPageProps) {
  ...
  const backHref = locale === "en" ? "/en/patuvanja" : "/patuvanja";
  ...
  return (
    <main className="px-6 py-16">
      <div className="mx-auto max-w-4xl">
        <Link href={backHref} className="text-lg text-amor-blue underline">
          ← {t("tripsCatalogTitle")}
        </Link>

        {trip.hero_image ? (
          <div className="relative mt-8 h-72 w-full overflow-hidden rounded-lg bg-amor-soft">
            <Image
              src={trip.hero_image}
              alt={title}
              fill
        ...
        {(earlyPrice || regularPrice) && (
          <div className="mt-8 space-y-2">
            {earlyPrice ? (
              <p className="text-2xl font-semibold text-amor-red">
                {locale === "en" ? "Early price" : "Попустна цена"}: {earlyPrice}
              </p>
            ) : null}
            {regularPrice ? (
              <p className="text-xl text-amor-blue">
